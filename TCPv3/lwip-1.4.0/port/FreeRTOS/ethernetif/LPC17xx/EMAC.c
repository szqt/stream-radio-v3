...
{
	uint32_t status = EMAC->IntStatus;
	static uint8_t LastRxProd = 0;
	static signed portBASE_TYPE xHigherPriorityTaskWoken;
	uint8_t licznik, NewRxProd;
	EMAC->IntClear = status;
	NewRxProd = EMAC->RxProduceIndex;

	if (status & (1 << 3)) {
		if (LastRxProd < NewRxProd) {
			for (licznik = 0; licznik < (NewRxProd - LastRxProd); licznik++)
				xSemaphoreGiveFromISR( semEthRx, &xHigherPriorityTaskWoken );
		} else {
			for (licznik = 0; licznik < (NUM_RX_FRAG - (LastRxProd - NewRxProd)); licznik++)
				xSemaphoreGiveFromISR( semEthRx, &xHigherPriorityTaskWoken );
		}
		LastRxProd = NewRxProd;
		LED_Toggle(7);
		cnt++;
	}
